...
namespace api.Service
{
    public class TmdbService : ITmdbService
    {
        ...
        public async Task<Movie?> GetMovieByIdAsync(int MovieId)
        {
            ...
            try
            {
                var result = await _httpClient.GetAsync($"{_config["TmdbSettings:BaseUrl"]}movie/{MovieId}?api_key={_config["TmdbSettings:ApiKey"]}&append_to_response=credits,videos,images,recommendations"); //?language=en-US
        ...
        public async Task<List<Movie?>> SearchMoviesByTitleAsync(string title, int page = 1) 
        {
            ...
            try
            {
                var result = await _httpClient.GetAsync($"{_config["TmdbSettings:BaseUrl"]}search/movie?api_key={_config["TmdbSettings:ApiKey"]}&query={title}");
                if (result.IsSuccessStatusCode)
                {
                    var content = await result.Content.ReadAsStringAsync();
